...
describe('functions', () => {
  ...
  const excludeRegexps = [
    new RegExp('/Api/not/Show/path$', 'i'),
    new RegExp('^https://www.domain.com/Not/show', 'i'),
  ];
  ...
  it(`isIgnored('/Api/Not/Show/login', excludeStrings, undefined) - 02`, () => {
    expect(isIgnored('/Api/Not/Show/login', excludeStrings, undefined)).toEqual(
      true
    );
  });
  ...
  it(`isIgnored('/some/api/not/show/path', undefined, excludeRegexps) - 03`, () => {
    expect(
      isIgnored('/some/api/not/show/pathS', undefined, excludeRegexps)
    ).toEqual(false);
  });

  it(`isIgnored('/some/api/not/show/path', undefined, excludeRegexps) - 04`, () => {
    expect(
      isIgnored('/Some/Api/Not/Show/Path', undefined, excludeRegexps)
    ).toEqual(true);
  });
});
